package com.fit.web.system.head;
...
import com.fit.common.utils.SpringMailUtil;
...
public class HeadController extends BaseController {
        ...
        if ("yes".endsWith(isAll)) {
            try {
                ...
                try {
                    for (int i = 0; i < userList.size(); i++) {
                        if (StringUtil.checkMobileNumber(userList.get(i).getString("PHONE"))) {            //手机号格式不对就跳过
                            if ("1".equals(TYPE)) {
            ...
            try {
                for (int i = 0; i < arrTITLE.length; i++) {
                    if (StringUtil.checkMobileNumber(arrTITLE[i])) {            //手机号式不对就跳过
                        if ("1".equals(TYPE)) {
            ...
            if (strEM.length == 4) {
                if ("yes".endsWith(isAll)) {
                    try {
                        ...
                        try {
                            for (int i = 0; i < userList.size(); i++) {
                                if (StringUtil.checkEmail(userList.get(i).getString("EMAIL"))) {        //邮箱格式不对就跳过
                                    SpringMailUtil.sendEmail(strEM[0], strEM[1], strEM[2], strEM[3], userList.get(i).getString("EMAIL"), TITLE, CONTENT, TYPE);//调用发送邮件函数
                                    count++;
                                } else {
                                    continue;
                                }
                    ...
                    try {
                        for (int i = 0; i < arrTITLE.length; i++) {
                            if (StringUtil.checkEmail(arrTITLE[i])) {        //邮箱格式不对就跳过
                                SpringMailUtil.sendEmail(strEM[0], strEM[1], strEM[2], strEM[3], arrTITLE[i], TITLE, CONTENT, TYPE);//调用发送邮件函数
                                count++;
                            } else {
                                continue;
                            }
    ...
    public ModelAndView saveSys() throws Exception {
        ...
        FileUtil.writeFile(Const.EMAIL, pd.getString("SMTP") + ",fh," + pd.getString("PORT") + ",fh," + pd.getString("EMAIL") + ",fh," + pd.getString("PAW"));    //写入邮件服务器配置
    ...
    public ModelAndView saveSys2() throws Exception {
        ...
        PageData pd = null;
        pd = this.getPageData();
        FileUtil.writeFile(Const.FWATERM, pd.getString("isCheck1") + ",fh," + pd.getString("fcontent") + ",fh," + pd.getString("fontSize") + ",fh," + pd.getString("fontX") + ",fh," + pd.getString("fontY"));    //文字水印配置
        FileUtil.writeFile(Const.IWATERM, pd.getString("isCheck2") + ",fh," + pd.getString("imgUrl") + ",fh," + pd.getString("imgX") + ",fh," + pd.getString("imgY"));    //图片水印配置
        Watermark.fushValue();
        mv.addObject("msg", "OK");
        mv.setViewName("save_result");
        return mv;
    ...
    public ModelAndView saveSys3() throws Exception {
        ...
        FileUtil.writeFile(Const.WEBSOCKET, pd.getString("WIMIP") + ",fh," + pd.getString("WIMPORT") + ",fh," + pd.getString("OLIP") + ",fh," + pd.getString("OLPORT") + ",fh," + pd.getString("FHsmsSound"));    //websocket配置
        mv.addObject("msg", "OK");
        mv.setViewName("save_result");
        return mv;
    }
