...
<body class="no-skin">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<a class="btn btn-sm btn-success" onclick="add('${r"${"}${objectNameUpper}_ID${r"}"}');">新增</a>
									</c:if>
									<c:if test="${r"${null != pd."}${objectNameUpper}_ID${r" && pd."}${objectNameUpper}_ID${r" != ''}"}">
									<a class="btn btn-sm btn-success" onclick="goSondict('${r"${pd.PARENT_ID}"}');">返回</a>
									</c:if>
								</td>
	...
	<!-- 下拉框 -->
	<script src="/assets/ace/js/chosen.jquery.js"></script>
	<!-- 日期框 -->
	...
	<script type="text/javascript">
		...
		function goSondict(${objectNameUpper}_ID){
			top.jzts();
			window.location.href="${ctx}${objectNameLower}/list.do?${objectNameUpper}_ID="+${objectNameUpper}_ID;
		};
		...
		function add(${objectNameUpper}_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '${ctx}${objectNameLower}/goAdd.do?${objectNameUpper}_ID='+${objectNameUpper}_ID;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
					 parent.location.href="${ctx}${objectNameLower}/listTree.do?${objectNameUpper}_ID=${r"${"}${objectNameUpper}_ID${r"}"}&dnowPage=${r"${page.currentPage}"}";
				}
				diag.close();
			 };
			 diag.show();
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "${ctx}${objectNameLower}/delete.do?${objectNameUpper}_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '${ctx}${objectNameLower}/goEdit.do?${objectNameUpper}_ID='+Id;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//导出excel
		function toExcel(){
			window.location.href='${ctx}${objectNameLower}/excel.do';
		}
	</script>
