...
<body class="no-skin">
	...
	<script type="text/javascript">
		...
		function goSondict(DEPARTMENT_ID){
			top.jzts();
			window.location.href="${ctx}department/list.do?DEPARTMENT_ID="+DEPARTMENT_ID;
		};
		...
		function add(DEPARTMENT_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '${ctx}department/goAdd.do?DEPARTMENT_ID='+DEPARTMENT_ID;
			 diag.Width = 700;
			 diag.Height = 546;
			 diag.CancelEvent = function(){ //关闭事件
				 if('none' == diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display){
					 parent.location.href="${ctx}department/listAllDepartment.do?DEPARTMENT_ID=${DEPARTMENT_ID}&dnowPage=${page.currentPage}";
				}
				diag.close();
			 };
			 diag.show();
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "${ctx}department/delete.do?DEPARTMENT_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '${ctx}department/goEdit.do?DEPARTMENT_ID='+Id;
			 diag.Width = 700;
			 diag.Height = 546;
			 diag.CancelEvent = function(){ //关闭事件
