...
<body class="no-skin">
	...
	<script type="text/javascript">
		...
		function add() {
			...
			diag.Drag = true;
			diag.Title = "新增";
			diag.URL = '${ctx}attached/goAdd.do';
			diag.Width = 500;
			diag.Height = 300;
		...
		function del(Id) {
			bootbox.confirm("确定要删除吗?", function (result) {
				if (result) {
					top.jzts();
					var url = "${ctx}attached/delete.do?ATTACHED_ID=" + Id + "&tm=" + new Date().getTime();
		...
		function edit(Id) {
			...
			diag.Drag = true;
			diag.Title = "编辑";
			diag.URL = '${ctx}attached/goEdit.do?ATTACHED_ID=' + Id;
			diag.Width = 800;
			diag.Height = 600;
			diag.CancelEvent = function () { //关闭事件
				if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
		...
		//导出excel
		function toExcel() {
			window.location.href = '${ctx}attached/excel.do';
		}
	</script>
</body>
</html>