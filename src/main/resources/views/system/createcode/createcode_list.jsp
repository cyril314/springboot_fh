...
<body class="no-skin">
	...
	<script type="text/javascript">
		...
		function productCode(CREATECODE_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="代码生成器";
			 diag.URL = '${ctx}createCode/goProductCode.do?CREATECODE_ID='+CREATECODE_ID;
			 diag.Width = 800;
			 diag.Height = 500;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "${ctx}createCode/delete.do?CREATECODE_ID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type:"POST",
								url:'${ctx}createCode/deleteAll.do?tm='+new Date().getTime(),
						    	data:{DATA_IDS:str},
								dataType:'json',
