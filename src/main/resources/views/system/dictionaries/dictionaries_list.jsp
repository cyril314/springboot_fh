...
<script type="text/javascript">
	...
	function goSondict(DICTIONARIES_ID){
		top.jzts();
		window.location.href="${ctx}dictionaries/list.do?DICTIONARIES_ID="+DICTIONARIES_ID;
	};
	...
	function add(DICTIONARIES_ID){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '${ctx}dictionaries/goAdd.do?DICTIONARIES_ID='+DICTIONARIES_ID;
		 diag.Width = 500;
		 diag.Height = 500;
		 diag.CancelEvent = function(){ //关闭事件
			 if('none' == diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display){
				 parent.location.href="${ctx}dictionaries/listAllDict.do?DICTIONARIES_ID=${DICTIONARIES_ID}&dnowPage=${page.currentPage}";
			}
			diag.close();
		 };
		 diag.show();
	...
	function del(Id){
		bootbox.confirm("确定要删除吗?", function(result) {
			if(result) {
				top.jzts();
				var url = "${ctx}dictionaries/delete.do?DICTIONARIES_ID="+Id+"&tm="+new Date().getTime();
	...
	function edit(Id){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '${ctx}dictionaries/goEdit.do?DICTIONARIES_ID='+Id;
		 diag.Width = 500;
		 diag.Height = 500;
		 diag.CancelEvent = function(){ //关闭事件
