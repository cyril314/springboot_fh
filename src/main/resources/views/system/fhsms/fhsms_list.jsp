...
<script type="text/javascript">
	...
	function del(ztid,STATUS,type,Id,SANME_ID){
		bootbox.confirm("确定要删除吗?", function(result) {
			if(type == "1" && STATUS == '2' && $("#"+ztid).html() == '<span class="label label-important arrowed-in">未读</span>'){
				top.readFhsms();//读取站内信时减少未读总数  <!-- readFhsms()函数在 WebRoot\assets\js\myjs\head.js中 -->
			}
			if(result) {
				top.jzts();
				var url = "${ctx}fhsms/delete.do?FHSMS_ID="+Id+"&tm="+new Date().getTime();
	...
	function viewx(ztid,STATUS,type,Id,SANME_ID){
		if(type == "1" && STATUS == '2' && $("#"+ztid).html() == '<span class="label label-important arrowed-in">未读</span>'){
		 ...
		 diag.Drag=true;
		 diag.Title ="站内信";
		 diag.URL = '${ctx}fhsms/goView.do?FHSMS_ID='+Id+'&TYPE='+type+'&SANME_ID='+SANME_ID+'&STATUS='+STATUS;
		 diag.Width = 600;
		 diag.Height = 460;
		 diag.CancelEvent = function(){ //关闭事件
			diag.close();
			...
			if(result) {
				...
				if(str==''){
					...
					if(msg == '确定要删除选中的数据吗?'){
						...
						$.ajax({
							type: "POST",
							url: '${ctx}fhsms/deleteAll.do?tm='+new Date().getTime(),
							data: {DATA_IDS:str},
							dataType:'json',
