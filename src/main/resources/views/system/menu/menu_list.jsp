...
<script type="text/javascript">
	...
	function parentReload(){
		if(null != ${MSG} && 'change' == ${MSG}){
			parent.location.href="${ctx}menu/listAllMenu.do?MENU_ID="+${MENU_ID};
		}else{
			//什么也不干
		}
	}
...
<script type="text/javascript">
	...
	function addmenu(MENU_ID){
		top.jzts();
		window.location.href="${ctx}menu/toAdd.do?MENU_ID="+MENU_ID;
	};

	//编辑菜单
	function editmenu(MENU_ID){
		top.jzts();
		window.location.href="${ctx}menu/toEdit.do?id="+MENU_ID;
	};
	...
	function delmenu(MENU_ID){
		bootbox.confirm("确定要删除此菜单吗?", function(result) {
			if(result) {
				var url = "${ctx}menu/delete.do?MENU_ID="+MENU_ID+"&guid="+new Date().getTime();
				top.jzts();
	...
	function editTb(MENU_ID){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑图标";
		 diag.URL = '${ctx}menu/toEditicon.do?MENU_ID='+MENU_ID;
		 diag.Width = 800;
		 diag.Height = 380;
