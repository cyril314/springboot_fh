...
<script type="text/javascript">
	...
	function addRole(pid){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '${ctx}role/toAdd.do?parent_id='+pid;
		 diag.Width = 222;
		 diag.Height = 100;
	...
	function editRole(ROLE_ID){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '${ctx}role/toEdit.do?ROLE_ID='+ROLE_ID;
		 diag.Width = 222;
		 diag.Height = 100;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function delRole(ROLE_ID,msg,ROLE_NAME){
		bootbox.confirm("确定要删除["+ROLE_NAME+"]吗?", function(result) {
			if(result) {
				var url = "${ctx}role/delete.do?ROLE_ID="+ROLE_ID+"&guid="+new Date().getTime();
				top.jzts();
	...
	function editRights(ROLE_ID){
		 ...
		 diag.Drag = true;
		 diag.Title = "菜单权限";
		 diag.URL = '${ctx}role/menuqx.do?ROLE_ID='+ROLE_ID;
		 diag.Width = 320;
		 diag.Height = 450;
		 diag.CancelEvent = function(){ //关闭事件
			diag.close();
	...
	function roleButton(ROLE_ID,msg){
		...
		if(msg == 'add_qx'){
		 ...
		 diag.Drag = true;
		 diag.Title = Title;
		 diag.URL = '${ctx}role/b4Button.do?ROLE_ID='+ROLE_ID+'&msg='+msg;
		 diag.Width = 330;
		 diag.Height = 450;
		 diag.CancelEvent = function(){ //关闭事件
			diag.close();
