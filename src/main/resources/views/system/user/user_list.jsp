...
function delUser(userId,msg){
	bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "${ctx}user/deleteU.do?USER_ID="+userId+"&tm="+new Date().getTime();
...
function editUser(user_id){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '${ctx}user/goEditU.do?USER_ID='+user_id;
	 diag.Width = 469;
	 diag.Height = 510;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		if(result) {
			...
			if(str==''){
				...
				if(msg == '确定要删除选中的数据吗?'){
					...
					$.ajax({
						type: "POST",
						url: '${ctx}user/deleteAllU.do?tm='+new Date().getTime(),
				    	data: {USER_IDS:str},
						dataType:'json',
...
function sendEmail(EMAIL){
	 ...
	 diag.Drag=true;
	 diag.Title ="发送电子邮件";
	 diag.URL = '${ctx}head/goSendEmail.do?EMAIL='+EMAIL;
	 diag.Width = 660;
	 diag.Height = 486;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function sendFhsms(username){
	 ...
	 diag.Drag=true;
	 diag.Title ="站内信";
	 diag.URL = '${ctx}fhsms/goAdd.do?username='+username;
	 diag.Width = 660;
	 diag.Height = 444;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function toExcel(){
	...
	window.location.href='${ctx}user/excel.do?keywords='+keywords+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_ID='+ROLE_ID;
}
...
function fromExcel(){
	 ...
	 diag.URL = '${ctx}user/goUploadExcel.do';
	 diag.Width = 300;
	 diag.Height = 150;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function viewUser(USERNAME){
	if('admin' == USERNAME){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '${ctx}user/view.do?USERNAME='+USERNAME;
	 diag.Width = 469;
	 diag.Height = 380;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
